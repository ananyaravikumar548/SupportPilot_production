  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-soft space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-bold text-slate-800">Support Agents Management</h2>
          <p className="text-xs text-slate-500">Manage agent allocations and details</p>
        </div>
            ...
            {agents.map((agent) => (
              <tr key={agent.id} className="hover:bg-slate-50">
                ...
                <td className="py-3 px-4">
                  <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-[10px] font-semibold">{agent.status}</span>
                </td>
                <td className="py-3 px-4 text-right">
                  <div className="flex justify-end gap-2 text-slate-400">
                    <button className="p-1 hover:text-emerald-600"><FiEdit2 /></button>
                    <button onClick={() => toast.info(`Deleted agent ${agent.name}`)} className="p-1 hover:text-red-500"><FiTrash2 /></button>
                  </div>
                </td>
              </tr>
            ))}
